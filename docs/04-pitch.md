...
- **For Professionals**: It features an Arrangement View and Piano Roll for manual, granular edits (like dragging notes or drawing automation curves).
...
- **AI Agent**: We use **Claude** with advanced Tool Calling. The AI doesn't modify the file blindly; instead, it uses an "Operation Layer" consisting of strict primitives (e.g., `addTrack`, `transpose`) and semantic macros to safely edit the document.

## Challenges we ran into

- **Rhythmic Stability**: Pure LLMs (like Claude) sometimes struggle with generating perfectly stable rhythmic MIDI arrays. We had to design a fallback strategy using template libraries for foundational tracks like drums and bass, leaving the AI to focus on melodies and harmonies.
...
- **Seamless AI Integration**: We're incredibly proud that our AI understands the *entire* global context of the song. If a user manually changes a note in the Piano Roll, the AI is immediately aware of it for the next chat prompt.
